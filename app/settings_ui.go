...
func (app *App) createSettingsContent() fyne.CanvasObject {
	...
	consoleToggle := modulesutil.NewM3Toggle(app.showConsole)
	consoleToggle.OnChange = func(checked bool) {
		app.showConsole = checked
		win.SetConsoleVisible(checked)
		_ = app.SaveConfig()
	}
	consoleRow := container.NewBorder(nil, nil, nil, consoleToggle,
		ctxt("Показывать консоль отладки", textPrimary, 14),
	)
	...
	systemSection := createSettingsSection(
		...
		container.NewVBox(
			...
			container.New(layout.NewCustomPaddedLayout(4, 4, 0, 0), consoleRow),
		),
	)
	...
	aboutLines := []fyne.CanvasObject{
		...
		func() fyne.CanvasObject {
			urlZovutil, _ := url.Parse("https://t.me/zovutil")
			return widget.NewRichText(
				&widget.TextSegment{
					Text:  "Форк оригинального проекта ",
					Style: widget.RichTextStyle{ColorName: theme.ColorNameDisabled, Inline: true},
				},
				&widget.HyperlinkSegment{Text: "zutil", URL: urlZovutil},
			)
		}(),
		settingsDivider(),
	...
	mainContent := container.NewVBox(header, colorSection, systemSection, updatesSection, packsSection, aboutSection)
