...
func (app *App) buildClickerContent() fyne.CanvasObject {
	...
	jitterDesc := widget.NewLabel("Вносит микро-случайность в интервалы кликов для имитации человеческого нажатия и защиты от бана на серверах.")
	jitterDesc.Wrapping = fyne.TextWrapWord
	...
	stateSection := createSettingsSection("Состояние", container.NewVBox(
		...
		container.New(layout.NewCustomPaddedLayout(0, 6, 0, 0), jitterDesc),
	))
