...
func (app *App) buildResourcePacksContent() fyne.CanvasObject {
	subTabs := []string{"ONLINE", "INSTALLED", "CONVERTER"}
	activeSubTab := "ONLINE"
	...
	converterView := app.buildConverterPacksView(func() {
		app.installedPacksDirty = true
	})

	if app.installedView != nil {
		mainContainer.Objects = []fyne.CanvasObject{onlineView, app.installedView, converterView}
	} else {
		mainContainer.Objects = []fyne.CanvasObject{onlineView, converterView}
	}
...
func installMcpack(name, downloadURL string, clonedOnly bool) error {
	...
	for _, packDir := range dirs {
		...
		for _, f := range zr.File {
			...
			if !strings.HasPrefix(filepath.Clean(destPath)+string(os.PathSeparator), filepath.Clean(packDir)+string(os.PathSeparator)) {
	...
	refresh := func() {
		go func() {
			...
			fyne.Do(func() {
				...
				if len(packs) == 0 {
					noPacksTxt := container.New(layout.NewCustomPaddedLayout(20, 20, 20, 20),
						container.NewCenter(ctxt("У вас нет установленных ресурспаков", textSecondary, 14)))
					rows = append(rows, noPacksTxt)
				} else {
					for _, p := range packs {
...
func (app *App) buildConverterPacksView(refreshFn func()) fyne.CanvasObject {
	title := ctxt("КОНВЕРТЕР РЕСУРСПАКОВ С БЕДРОКА ДЛЯ МКПЕ", accentRed, 15)
	title.TextStyle = fyne.TextStyle{Bold: true, Monospace: true}

	subtitle := widget.NewLabel("Выберите .mcpack скайбокса для автоматической конвертации кубмапа и импорта в Minecraft.")
	subtitle.Wrapping = fyne.TextWrapWord

	convertBtn := widget.NewButtonWithIcon("Выбрать .mcpack скайбокса", theme.UploadIcon(), func() {
		fd := dialog.NewFileOpen(func(reader fyne.URIReadCloser, err error) {
			...
			stepLabel := widget.NewLabel("Инициализация конвертации...")
			stepLabel.Wrapping = fyne.TextWrapWord
			stepLabel.TextStyle = fyne.TextStyle{Bold: true}

			logText := widget.NewLabel("• Подготовка к обработке скайбокса...")
			logText.Wrapping = fyne.TextWrapWord

			modalContent := container.NewVBox(
				ctxt("КОНВЕРТАЦИЯ СКАЙБОКСА", accentRed, 16),
				container.New(layout.NewCustomPaddedLayout(4, 0, 0, 0), stepLabel),
				container.New(layout.NewCustomPaddedLayout(6, 0, 0, 0), pBar),
				container.New(layout.NewCustomPaddedLayout(8, 0, 0, 0), logText),
			)

			paddedModal := container.New(layout.NewCustomPaddedLayout(16, 16, 16, 16), modalContent)
			progressDlg := dialog.NewCustomWithoutButtons("Обработка Скайбокса", paddedModal, app.win)
			progressDlg.Show()

			go func() {
				displayName, err := ConvertAndImportSkyboxPack(path, app.useClonedMinecraft, func(step string, percent float64) {
					fyne.Do(func() {
						pBar.SetValue(percent)
						stepLabel.SetText(fmt.Sprintf("[%d%%] %s", int(percent*100), step))
						logText.SetText(fmt.Sprintf("• %s", step))
					})
				})
				...
				fyne.Do(func() {
					...
					if err != nil {
						...
						dialog.ShowInformation("Успешно!", fmt.Sprintf("Скайбокс \"%s\" успешно сконвертирован, отчищен от облаков и импортирован в Minecraft!", displayName), app.win)
						if refreshFn != nil {
							refreshFn()
						}
					}
	...
	cardContent := container.NewVBox(
		title,
		subtitle,
		container.New(layout.NewCustomPaddedLayout(12, 0, 0, 0), convertBtn),
	)

	cardBg := canvas.NewRectangle(bgCard)
	cardBorder := canvas.NewRectangle(color.Transparent)
	cardBorder.StrokeWidth = 1
	cardBorder.StrokeColor = borderRed

	paddedCard := container.New(layout.NewCustomPaddedLayout(16, 16, 16, 16), cardContent)
	converterCard := container.NewStack(cardBg, paddedCard, cardBorder)

	return container.NewVScroll(container.New(layout.NewCustomPaddedLayout(16, 16, 16, 16), converterCard))
}
