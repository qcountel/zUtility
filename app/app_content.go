...
func (app *App) createContent(proc *win.Process) (fyne.CanvasObject, []module.Module, error) {
	...
	if len(existingMods) > 0 {
		...
		mods = []module.Module{
			...
			app.createInstantHotbarModule(proc),
		}
	}
...
func (app *App) buildToolbar(mods []module.Module, onTabSwitch func(int)) fyne.CanvasObject {
	...
	playBtn := container.NewStack(playBg, container.NewCenter(playIcon), playTap)

	toolbarContent := container.NewBorder(nil, nil, tabsRow, playBtn)
...
func (app *App) buildModulesContent(mods []module.Module) fyne.CanvasObject {
	subTabs := []string{"ALL", "GAMEPLAY", "VISUAL", "CONTROLLER"}
	activeSubTab := "ALL"
	...
	rebuildList := func(tab string) {
		var rows []fyne.CanvasObject
		for _, m := range mods {
			cat := getModuleCategory(m.Name())
			if tab == "ALL" || cat == tab {
		...
		if scrollableList != nil {
			scrollableList.Refresh()
		}
	}

	rebuildList(activeSubTab)
	...
	for i, name := range subTabs {
		...
		lblPadded := container.New(layout.NewCustomPaddedLayout(9, 7, 13, 13), lbl)
		btnCol := container.NewVBox(lblPadded, underline)
	...
	return container.NewBorder(headerWithBorder, nil, nil, nil, scrollableList)
}
...
func (app *App) buildModuleRow(m module.Module) fyne.CanvasObject {
	...
	leftPart := container.NewHBox(
		...
		container.New(layout.NewCustomPaddedLayout(0, 0, 4, 0), nameText),
	)
	...
	return container.NewVBox(
		container.NewStack(rowBg, paddedRow),
		rowBorder,
	)
}
...
func (app *App) createInstantHotbarModule(proc *win.Process) module.Module {
	return modules.InstantHotbar{Process: proc, Error: app.onError("instant_hotbar"), AfterChange: app.autoSave, Window: app.win}.Create()
}
