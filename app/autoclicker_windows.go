...
var (
	...
	acProcGetForegroundWindow      = acUser32DLL.NewProc("GetForegroundWindow")
	acProcGetWindowThreadProcessId = acUser32DLL.NewProc("GetWindowThreadProcessId")
	acProcGetClassNameW            = acUser32DLL.NewProc("GetClassNameW")
	acProcFindWindowEx             = acUser32DLL.NewProc("FindWindowExW")
	acProcPostThreadMsg            = acUser32DLL.NewProc("PostThreadMessageW")
...
func (app *App) getAutoClicker() *autoClicker {
	...
	ac := newAutoClicker()
	if app.tr != nil && app.tr.Process() != nil {
		ac.SetTargetPID(app.tr.Process().PID)
	}
...
func (ac *autoClicker) Close() {
	...
	if tid != 0 {
		_, _, _ = acProcPostThreadMsg.Call(uintptr(tid), 0x0012 /*WM_QUIT*/, 0, 0)
	}
}
...
func (ac *autoClicker) SetTargetPID(pid uint32) {
	ac.mu.Lock()
	ac.targetPID = pid
	ac.mu.Unlock()
}

func (ac *autoClicker) isMinecraftActive() bool {
	ac.mu.Lock()
	pid := ac.targetPID
	ac.mu.Unlock()
	if pid == 0 {
	...
	hwnd, _, _ := acProcGetForegroundWindow.Call()
	if hwnd == 0 {
		return false
	}
	var forePID uint32
	acProcGetWindowThreadProcessId.Call(hwnd, uintptr(unsafe.Pointer(&forePID)))
	if forePID == pid {
		return true
	}

	var className [256]uint16
	r, _, _ := acProcGetClassNameW.Call(hwnd, uintptr(unsafe.Pointer(&className[0])), uintptr(len(className)))
	if r > 0 {
		classStr := windows.UTF16ToString(className[:])
		if classStr == "ApplicationFrameWindow" {
			coreWindowClass, _ := windows.UTF16PtrFromString("Windows.UI.Core.CoreWindow")
			childHwnd, _, _ := acProcFindWindowEx.Call(hwnd, 0, uintptr(unsafe.Pointer(coreWindowClass)), 0)
			if childHwnd != 0 {
				var childPID uint32
				acProcGetWindowThreadProcessId.Call(childHwnd, uintptr(unsafe.Pointer(&childPID)))
				if childPID == pid {
					return true
				}
			}
...
func (ac *autoClicker) clickLoop(ctx context.Context) {
	...
	for {
		...
		if !ac.isPhysicalHeld(mode) || !ac.isMinecraftActive() {
