...
func (app *App) SaveConfig() error {
	...
	for _, m := range app.modules {
		...
		if colorer, ok := m.(interface{ Colors() (float64, float64, float64) }); ok {
			r, g, b := colorer.Colors()
...
func (app *App) PreloadTheme() error {
	configPath, err := getConfigPath()
	if err != nil {
		return fmt.Errorf("get config path: %w", err)
	}
	...
	app.showConsole = settings.ShowConsole
	win.SetConsoleVisible(settings.ShowConsole)
...
func (app *App) LoadConfig() error {
	...
	app.loadingConfig.Store(true)
	defer app.loadingConfig.Store(false)
	...
	for _, cfg := range settings.Modules {
		for _, m := range app.modules {
			if canonicalModuleName(m.Name()) == canonicalModuleName(cfg.Name) {
				...
				if setter, ok := m.(interface{ SetColors(float64, float64, float64) }); ok {
					setter.SetColors(cfg.ColorR, cfg.ColorG, cfg.ColorB)
				}
